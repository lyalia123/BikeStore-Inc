...
def load_data():
    ...
    for table, filename in tables:
        ...
        try:
            ...
            if error_count > 0:
                print(f"⚠️ Failed to load {error_count} rows into {table}")
...
def run_data_generator():
    ...
    generator.run_continuous_generation(interval=15)

def run_superset_setup():
    """Настройка подключения Superset к БД"""
    print("⚙️ Настройка подключения Superset...")
    time.sleep(10)  # Ждем запуска Superset
    os.system("python superset_setup.py")

def main():
    print("=== Assignment #3 - BikeStore Analytics Dashboard ===")
    print("1. Создание базы данных и таблиц")
    print("2. Запуск Apache Superset")
    print("3. Настройка подключения к БД")
    print("4. Запуск генератора данных")
    print()
    
    # Шаг 1: Настройка базы данных
    setup_database()
    
    # Шаг 2: Запуск Superset в отдельном потоке
    superset_thread = threading.Thread(target=run_superset)
    superset_thread.daemon = True
    superset_thread.start()
    
    # Шаг 3: Настройка подключения Superset
    time.sleep(30)  # Ждем полного запуска Superset
    superset_setup_thread = threading.Thread(target=run_superset_setup)
    superset_setup_thread.daemon = True
    superset_setup_thread.start()
    
    # Шаг 4: Запуск генератора данных
    time.sleep(5)
    generator_thread = threading.Thread(target=run_data_generator)
    generator_thread.daemon = True
    generator_thread.start()
    
    print("\n🎉 Система запущена!")
    print("🌐 Superset: http://localhost:8088")
    print("📊 Генератор данных: добавляет новые заказы каждые 15 секунд")
    print("🗄️ База данных: создана и наполнена тестовыми данными")
    print()
